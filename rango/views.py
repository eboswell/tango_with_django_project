...
def about(request):
    ...
    context_dict = {'boldmessage': 'This tutorial has been put together by Elizabeth Boswell.'}
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if user_form.is_valid() and profile_form.is_valid():
            ...
            #Once hashed, we can update the users object.
            user.set_password(user.password)
            user.save()

            #Now sort out the UserProfile instance.
            #Since we need to set the user attribute ourselves,
            #we set commit=False. This delays saving the model until we're
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        #to request.POST('<variable>') because the request.POST.get('<variable>')
...
#user the login_required() decorator to ensure only those logged in can access the
#view.
@login_required
